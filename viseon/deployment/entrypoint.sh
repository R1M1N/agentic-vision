...
start_inference() {
    ...
    exec gunicorn \
        ...
        viseon.inference.server:create_app \
        --port ${INFERENCE_PORT}
}
...
start_training() {
    ...
    exec python -m viseon.training.yolo_trainer \
        --host 0.0.0.0 \
        --port 8080 \
        --log-level ${LOG_LEVEL,,}
}
...
start_development() {
    ...
    if [ "$SERVICE_TYPE" = "inference" ]; then
        uvicorn viseon.inference.server:create_app \
            --host 0.0.0.0 \
            --port ${INFERENCE_PORT} \
            --reload \
            --log-level ${LOG_LEVEL,,}
...
# Main execution
main() {
    log "Starting viseon ${SERVICE_TYPE} service..."
    log "Configuration:"
